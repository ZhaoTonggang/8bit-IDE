...
exports.highlightSearch = exports.reloadWorkspaceFile = exports.emulationHalted = exports.getSaveState = exports
	.getTestOutput = exports.setTestInput = exports.startUI = exports.getPlatformAndRepo = exports.setupSplits = exports
	.haltEmulation = exports.setFrameRateUI = exports.clearBreakpoint = exports.runToPC = exports.setupBreakpoint =
	exports.lastDebugState = exports.compparams = exports.projectWindows = exports.current_project = exports.platform =
	exports.repo_id = exports.store_id = exports.platform_id = exports.qs = void 0;
...
function requestPersistPermission(interactive, failureonly) {
	if (navigator.storage && navigator.storage.persist) {
		navigator.storage.persist().then(persistent => {
			...
			if (persistent) {
				interactive && !failureonly && alertInfo(
					"Your browser says it will persist your local file edits, but you may want to back up your work anyway."
					);
			} else {
				interactive && alertError(
					"Your browser refused to expand the peristent storage quota. Your edits may not be preserved after closing the page."
					);
			}
		});
	} else {
		interactive && alertError(
		"Your browser may not persist edits after closing the page. Try a different browser.");
	}
}
...
async function initProject() {
	...
	exports.current_project = new project_1.CodeProject(newWorker(), exports.platform_id, exports.platform,
		filesystem);
...
async function getSkeletonFile(fileid) {
	...
	try {
		return await $.get("presets/" + (0, util_1.getBasePlatform)(exports.platform_id) + "/skeleton." + ext,
			'text');
	} catch (e) {
...
function _uploadNewFile(e) {
	...
	uploadFileElem.change((e) => {
		handleFileUpload(file.files);
	});
			...
			if (index > 2) {
				...
				bootbox.confirm({
					...
					buttons: {
						...
						cancel: {
							label: "Include/Link With Project Later"
						},
					},
					callback: (result) => {
						if (result)
...
async function getLocalFilesystem(repoid) {
	...
	return {
		getFileData: async (path) => {
			...
			let fileHandle = await dirHandle.getFileHandle(path, {
				create: false
			});
...
async function importProjectFromGithub(githuburl, replaceURL) {
	...
	if (existing && !confirm("You've already imported " + urlparse.repopath +
			" -- do you want to replace all local files?")) {
		return;
	}
	...
	return gh.import(githuburl).then((sess1) => {
		...
		exports.qs = {
			repo: sess.repopath
		}; // file:sess.mainPath, platform:sess.platform_id};
		setWaitDialog(false);
...
function _publishProjectToGithub(e) {
	if (exports.repo_id) {
		if (!confirm("This project (" + exports.current_project.mainPath +
				") is already bound to a Github repository. Do you want to re-publish to a new repository? (You can instead choose 'Push Changes' to update files in the existing repository.)"
				))
			return;
	}
	var modal = $("#publishGithubModal");
	var btn = $("#publishGithubButton");
	$("#githubRepoName").val((0, util_1.getFilenamePrefix)((0, util_1.getFilenameForPath)(exports.current_project
		.mainPath)));
	modal.modal('show');
	...
	return new Promise((resolve, reject) => {
		...
		for (var f of files) {
			...
			if (f.additions || f.deletions || f.changes) {
				msg += " (" + f.additions + " additions, " + f.deletions + " deletions, " + f.changes +
					" changes)";
			};
			msg += "<br/>";
		}
...
function _deleteRepository() {
	...
	if (!ghurl)
		return;
	bootbox.prompt("<p>Are you sure you want to delete this repository (" + DOMPurify.sanitize(ghurl) +
		") from browser storage?</p><p>All changes since last commit will be lost.</p><p>Type DELETE to proceed.<p>",
		(yes) => {
...
function _downloadCassetteFile_apple2(e) {
	...
	(0, util_1.loadScript)('lib/c2t.js').then(() => {
		...
		FS.writeFile(rompath, current_output, {
			encoding: 'binary'
		});
		var args = ["-2bc", rompath + ',' + addr.toString(16), audpath];
		c2t.callMain(args);
		var audout = FS.readFile(audpath, {
			'encoding': 'binary'
		});
		if (audout) {
			var blob = new Blob([audout], {
				type: "audio/wav"
			});
			(0, file_saver_1.saveAs)(blob, audpath);
			stdout +=
				"Then connect your audio output to the cassette input, turn up the volume, and play the audio file.";
			alertInfo(stdout);
		}
	});
}
...
function _downloadCassetteFile_vcs(e) {
	(0, util_1.loadScript)('lib/makewav.js').then(() => {
		...
		_makewav.ready.then((makewav) => {
			...
			console.log(stdout);
			let FS = makewav['FS'];
			let audout = FS.readFile(audpath, {
				'encoding': 'binary'
			});
			if (audout) {
				let blob = new Blob([audout], {
					type: "audio/wav"
				});
				(0, file_saver_1.saveAs)(blob, audpath);
				stdout +=
	...
	if (wnd && wnd.setText) {
		...
		$.get("presets/" + (0, util_1.getBasePlatform)(exports.platform_id) + "/" + fn, (text) => {
			...
			.fail(() => {
				if (exports.repo_id)
					alertError(
						"Can only revert built-in examples. If you want to revert all files, You can pull from the repository."
						);
				else
...
function _downloadROMImage(e) {
	...
	if (exports.platform.getDownloadFile) {
		...
		var blob = new Blob([current_output], {
...
function _downloadSourceFile(e) {
	...
	(0, file_saver_1.saveAs)(blob, getCurrentEditorFilename(), {
		autoBom: false
	});
}
...
async function _downloadProjectZipFile(e) {
	...
	zip.generateAsync({
		...
		(0, file_saver_1.saveAs)(content, getCurrentMainFilename() + "-" + (0, util_1.getBasePlatform)(
			exports.platform_id) + ".zip");
	});
}
...
function _downloadSymFile(e) {
	...
	var prefix = (0, util_1.getFilenamePrefix)(getCurrentMainFilename());
	(0, file_saver_1.saveAs)(symfile.blob, prefix + symfile.extension, {
		autoBom: false
	});
}
async function _downloadAllFilesZipFile(e) {
		...
		function populateExamples(sel) {
			...
			for (var i = 0; i < PRESETS.length; i++) {
				...
				sel.append($("<option />").val(preset.id).text(name).attr('selected', isCurrentPreset ? 'selected' :
					null));
				if (isCurrentPreset)
			...
			if (hasLocalStorage && !isElectron) {
				...
				if (repos) {
					for (let repopath in repos) {
						...
						if (repo.platform_id && (0, util_1.getBasePlatform)(repo.platform_id) == (0, util_1
								.getBasePlatform)(exports.platform_id)) {
							if (n++ == 0)
		...
		async function populateFiles(sel, category, prefix, foundFiles) {
			...
			for (var i = 0; i < keys.length; i++) {
				...
				if (key.startsWith(prefix) && !foundFiles[key]) {
					if (numFound++ == 0)
						sel.append($("<option />").text("------- " + category + " -------").attr('disabled',
							'true'));
					var name = key.substring(prefix.length);
					sel.append($("<option />").val(key).text(name).attr('selected', (key == exports.current_project
						.mainPath) ? 'selected' : null));
				}
			}
		}
		...
		function getErrorElement(err) {
			var span = $('<p/>');
			if (err.path != null) {
				var s = err.line ? err.label ? `(${err.path} @ ${err.label})` : `(${err.path}:${err.line})` :
					`(${err.path})`;
				var link = $('<a/>').text(s);
				var path = err.path;
		...
		function measureBuildTime() {
			if (window['ga'] && measureTimeLoad) {
				...
				ga('send', 'timing', 'load', exports.platform_id, (measureTimeLoad.getTime() - measureTimeStart
				.getTime()));
				ga('send', 'timing', 'build', exports.platform_id, (measureTimeBuild.getTime() - measureTimeLoad
					.getTime()));
		...
		function _addIncludeFile() {
			...
			if (fn.endsWith(".c") || tool == 'sdcc' || tool == 'cc65' || tool == 'cmoc' || tool == 'smlrc')
				addFileToProject("Header", ".h", (s) => {
					return '#include "' + s + '"';
				});
			else if (tool == 'dasm' || tool == 'zmac')
				addFileToProject("Include", ".inc", (s) => {
					return '\tinclude "' + s + '"';
				});
				...
				addFileToProject("Verilog", ".v", (s) => {
					return '`include "' + s + '"';
				});
			else if (tool == 'wiz')
				addFileToProject("Include", ".wiz", (s) => {
					return 'import "' + s + '";';
				});
			else if (tool == 'ecs')
				addFileToProject("Include", ".ecs", (s) => {
					return 'import "' + s + '"';
				});
			else
		...
		function _addLinkFile() {
			...
			var tool = exports.platform.getToolForFilename(fn);
			if (fn.endsWith(".c") || tool == 'sdcc' || tool == 'cc65' || tool == 'cmoc' || tool == 'smlrc')
				addFileToProject("Linked C (or .s)", ".c", (s) => {
					return '//#link "' + s + '"';
				});
			else if (fn.endsWith("asm") || fn.endsWith(".s") || tool == 'ca65' || tool == 'lwasm')
				addFileToProject("Linked ASM", ".inc", (s) => {
					return ';#link "' + s + '"';
				});
			else
		...
		function setupDebugControls() {
			...
			if (exports.platform.restartAtPC) {
				uitoolbar.add('ctrl+alt+/', 'Restart at Cursor', 'glyphicon-play-circle', restartAtCursor).prop('id',
					'dbg_restartatline');
			}
			uitoolbar.newGroup();
			uitoolbar.grp.prop('id', 'debug_bar');
			if (exports.platform.runEval) {
				uitoolbar.add('ctrl+alt+e', 'Reset and Debug', 'glyphicon-fast-backward', resetAndDebug).prop('id',
					'dbg_restart');
			}
			if (exports.platform.stepBack) {
				uitoolbar.add('ctrl+alt+b', 'Step Backwards', 'glyphicon-step-backward', runStepBackwards).prop('id',
					'dbg_stepback');
			}
			...
			if (exports.platform.runUntilReturn) {
				uitoolbar.add('ctrl+alt+o', 'Step Out of Subroutine', 'glyphicon-hand-up', runUntilReturn).prop('id',
					'dbg_stepout');
			}
			if (exports.platform.runToVsync) {
				uitoolbar.add('ctrl+alt+n', 'Next Frame/Interrupt', 'glyphicon-forward', singleFrameStep).prop('id',
					'dbg_tovsync');
			}
			...
			if (exports.platform_id.startsWith('apple2') || exports.platform_id.startsWith(
				'vcs')) // TODO: look for function
			...
			if (exports.platform.showHelp) {
				let {
					li,
					a
				} = newDropdownListItem('help__' + exports.platform_id, platform_name + ' Help');
				$("#help_menu").append(li);
			...
			if (toolhelpurl) {
				let {
					li,
					a
				} = newDropdownListItem('help__' + tool, tool + ' Help');
				$("#help_menu").append(li);
		...
		function setupReplaySlider() {
			...
			$("#replay_bar").show();
			uitoolbar.add('ctrl+alt+0', 'Start/Stop Replay Recording', 'glyphicon-record', _toggleRecording).prop('id',
				'dbg_record');
		}

		function isLandscape() {
			try {
				var object = window.screen['orientation'] || window.screen['msOrientation'] || window.screen[
					'mozOrientation'] || null;
				if (object) {
		...
		async function showWelcomeMessage() {
			if (userPrefs.shouldCompleteTour()) {
				...
				var steps = [{
						...
						content: "您目前在\"<b>" + exports.platform_id + "</b>\" 平台上。 您可以从菜单切换不同平台！"
					},
					{
						element: "#preset_select",
						title: "Project Selector",
						content: "您可以选择不同的代码示例、创建自己的文件或从 GitHub 导入项目。"
					},
					{
						element: "#workspace",
						title: "Code Editor",
						content: is_vcs ? "在编辑器中输入您的 6502 汇编代码，它将被实时汇编。" : "在编辑器中输入您的源代码，它将被实时编译。"
					},
					{
						element: "#emulator",
						placement: 'left',
		...
		function installGAHooks() {
			if (window['ga']) {
				...
				ga('send', 'pageview', location.pathname + '?platform=' + exports.platform_id + (exports.repo_id ? (
					'&repo=' + exports.repo_id) : ('&file=' + exports.qs.file)));
			}
		}
		async function startPlatform() {
			...
			if (!exports.qs.file) {
				...
				if (!defaultfile) {
					alertError(
						"There is no default main file for this project. Try selecting one from the pulldown.");
				}
			}
			// legacy vcs stuff
			if (exports.platform_id == 'vcs' && exports.qs.file.startsWith('examples/') && !exports.qs.file
				.endsWith('.a')) {
				exports.qs.file += '.a';
			}
		...
		async function loadAndStartPlatform() {
			try {
				var module = await (0, _index_1.importPlatform)((0, util_1.getRootBasePlatform)(exports
					.platform_id));
				console.log("starting platform", exports.platform_id); // loaded required <platform_id>.js file
				await startPlatform();
				document.title = document.title + " [" + exports.platform_id + "] - " + (exports.repo_id ? ('[' +
					exports.repo_id + '] - ') : '') + exports.current_project.mainPath;
			} catch (e) {
				console.log(e);
